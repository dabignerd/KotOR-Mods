...
void main() {
	ActionPlayAnimation(201, 1.0, 0.0);
       if(GetGlobalBoolean("K1R_KORR_DROIDS") == 1) { return; }
	if ((!sub1(OBJECT_SELF, intGLOB_1))) {
		...
		object oKor37_bridge5 = GetObjectByTag("kor37_bridge5", 0);
		if (!GetIsDead(oKor37_bridge1) || !GetIsDead(oKor37_bridge2) || !GetIsDead(oKor37_bridge3) || !GetIsDead(oKor37_bridge4) || !GetIsDead(oKor37_bridge5)) {
			...
			if (GetIsObjectValid(oKor37_bridge5)) {
				CancelCombat(oKor37_bridge5);
				AssignCommand(oKor37_bridge5, ClearAllEffects());
				AssignCommand(oKor37_bridge5, ClearAllActions());
				ChangeToStandardFaction(oKor37_bridge5, 5);
			}
